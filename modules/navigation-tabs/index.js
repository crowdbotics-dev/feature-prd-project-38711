import React, { useEffect, useState, Fragment, useContext } from "react"; // @ts-ignore

import { createBottomTabNavigator } from "@react-navigation/bottom-tabs";
import PropTypes from "prop-types";
import { View, Text, StyleSheet } from "react-native"; // @ts-ignore
...
function MyTabs({
  ...
  useEffect(async () => {
    if (tabList.length !== 0) {
      ...
      if (riseCenteredBtn && check) {
        ...
        value.tabBarItemStyle = {
          marginTop: -30,
          backgroundColor: value?.tabBarItemStyle?.backgroundColor || tabBarStyle?.backgroundColor || "#fff",
          height: 70,
          borderRadius: 60
        };
      }
  ...
  return <Fragment>
      {!tabData ? <View style={styles.warning}><Text style={styles.text}>No Data Found!</Text></View> : <Tab.Navigator screenOptions={{
      ...
      tabBarStyle: tabBarStyle
        ...
        return <Tab.Screen key={index} name={tab?.name} component={tab?.component} options={{
